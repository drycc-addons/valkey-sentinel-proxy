...
curl -s --data-binary @_dist/valkey-sentinel-proxy -H "Content-Type: application/octet-stream" -H "${GITHUB_AUTH_HEAD}" -X POST "${GITHUB_ASSET_UPLOAD_URL}?name=valkey-sentinel-proxy-${CI_SYSTEM_PLATFORM}"